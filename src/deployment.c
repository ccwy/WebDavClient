...
// ----------------------------------------------------------------------
// 1. 手写轻量 IBindStatusCallback COM 接口实现下载百分比进度
// ----------------------------------------------------------------------
typedef struct {
...
// 状态更新辅助函数 (使用 PostMessageW 彻底消除死锁)
static void UpdateStatusW(const WCHAR* format, ...) {
    if (!format) return;

    // 自动容错：将 ini 模版里可能的 %S 格式化符替换为兼容 wchar_t* 的 %s
    WCHAR safeFmt[512] = { 0 };
